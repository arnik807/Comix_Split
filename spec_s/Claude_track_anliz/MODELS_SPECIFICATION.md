...
### 4.4 TPSMM — motion transfer (подготовлено, не в пайплайне)
...
| **Статус** | Модели готовы; `anim/animate_tpsmm.py` написан; `anim_pipeline` пока не вызывает |

**Под железо: ⭐⭐** — ~50–120 MB INT8, CPU inference возможен, но медленно. Имеет смысл после интеграции сегментации.
...
| TPSMM (будущее) | ONNX CPU | средняя | Медленно | Driving-зависимо |
