...
PNG панели из output/
  ...
  → anim/animate_depthflow.py  или  anim/animate_opencv.py
...
| Тесты | pytest | — | 25 тестов в 7 файлах |
...
│   ├── animate_tpsmm.py      # Motion transfer [Unreleased]
...
|----------|-------|-----------|
| `/api/process` | POST | Детекция панелей (только в память) |
| `/api/export` | POST | Сохранить PNG (rect или polygon) |
| `/api/upscale` | POST | Апскейл папки панелей |
| `/api/animate` | POST | Анимировать панели → MP4 |
...
| `/api/path/pick` | POST | Нативный диалог выбора пути |
| `/api/split/options` | GET | Доступные детекторы + статус |
| `/api/upscale/options` | GET | Доступные backend апскейла |
| `/api/models/setup` | GET | Статус установки моделей |
