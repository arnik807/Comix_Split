...
| **B2** | TPSMM motion transfer | 🟡 **Код готов** — интеграция в пайплайн |
...
**Текущее состояние:** код готов (`anim/animate_tpsmm.py`), модели скачаны и квантованы (`models/anim/tpsmm/`), UI и API поля добавлены — **не подключено** к `anim_pipeline.py`.

### Что осталось

| # | Задача |
|---|--------|
| B2.1 | Интегрировать `animate_tpsmm.py` в `anim_pipeline.process_panel()` как режим `tpsmm` |
| B2.2 | Подключить `anim/segment.py` (MobileSAM по bbox) для отделения персонажа от фона |
| B2.3 | Проверить качество на панелях комикса, подобрать дефолтный driving video |
| B2.4 | Предупреждение в UI: «медленно, ~N мин/панель на CPU» |
| B2.5 | Тесты: `tests/test_tpsmm.py` |
